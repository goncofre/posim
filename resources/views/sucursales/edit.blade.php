...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <h1>Editar Ubicación: {{ $sucursal->nombre }}</h1>
            <hr>
                    ...
                    <form action="{{ route('sucursales.update', $sucursal) }}" method="POST">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="tipo" name="tipo" class="form-select @error('tipo') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tipos as $key => $value)
                                        <option value="{{ $key }}" {{ old('tipo', $sucursal->tipo) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('comuna') is-invalid @enderror" id="comuna" name="comuna" value="{{ old('comuna', $sucursal->comuna) }}">
                                @error('comuna') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-md-6">
                                <label for="region" class="form-label">Región</label>
                                <input type="text" class="form-control @error('region') is-invalid @enderror" id="region" name="region" value="{{ old('region', $sucursal->region) }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $sucursal->email) }}">
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            ...
                            <button type="submit" class="btn btn-warning text-dark"><i class="fas fa-sync-alt"></i> Actualizar Ubicación</button>
                        </div>
                    </form>
                </div>
            </div>
